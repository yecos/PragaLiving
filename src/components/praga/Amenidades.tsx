...
export default function Amenidades() {
  ...
  const amenities = useMemo<Amenity[]>(() => {
    ...
    return configuredItems.map((item: Partial<Amenity> & { image?: string }, index: number) => {
      ...
      const configuredImages = Array.isArray(item.images) && item.images.length > 0
        ? item.images
        : typeof item.image === 'string' && item.image.trim()
          ? [{ src: item.image.trim(), alt: `${item.name || fallback.name} de PRAGA Living`, label: item.name || fallback.name }, ...fallback.images.slice(1)]
          : fallback.images
      ...
      return {
        ...
        images: configuredImages as AmenityImage[],
      }
    })
  }, [amenConfig?.items])
